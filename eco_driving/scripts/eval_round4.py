"""Round-4 evaluation of models/sac_seed0_round4 against baseline on the same
10 fixed scenarios, with paired fuel delta restricted to legal-arrival-on-both-
sides scenarios, plus guard-activation rate, and explicit acceptance-criteria
checks for this round. See CHANGES.md ("Round 4") for context and results.
"""
import numpy as np

from eco_driving.config import EnvConfig
from eco_driving.baseline.idm_driver import IDMBaselineDriver
from eco_driving.scripts.evaluate import (SCENARIO_SEEDS, load_policy, rollout_baseline,
                                           summarize_metric, policy_action)
from eco_driving.envs import EcoDrivingEnv

cfg = EnvConfig()
model_dir = "models/sac_seed0_round4"
model, vecnorm = load_policy(model_dir)
driver = IDMBaselineDriver(cfg)


def rollout_policy_with_guard_tracking(cfg, seed, model, vecnorm):
    ...
    while not (terminated or truncated):
        a = policy_action(model, vecnorm, obs)
        obs, r, terminated, truncated, info = env.step(np.array([a], dtype=np.float32))
        n_steps += 1
        if info.get("forced_brake") or info.get("forced_brake_leader"):
            n_guard += 1
        if info["v"] < 0.3:
            stop_steps += 1
        max_jerk = max(max_jerk, abs(info["jerk"]))
        total_fuel += info["fuel_mL"]
        info_last = info
    arrived = bool(info_last.get("arrived", False)) and terminated and \
        not info_last.get("red_run", False) and not info_last.get("collision", False)
    summary = dict(seed=seed, arrived=arrived, timeout=bool(truncated),
                    red_run=bool(info_last.get("red_run", False)),
                    collision=bool(info_last.get("collision", False)),
                    travel_time_s=env.t, total_fuel_mL=total_fuel, stop_steps=stop_steps,
                    max_abs_jerk=max_jerk, guard_rate=n_guard / max(n_steps, 1))
    return summary
...
for s in SCENARIO_SEEDS:
    ...
    p_summary = rollout_policy_with_guard_tracking(cfg, s, model, vecnorm)
    ...
    print(f"scenario {s}: "
          ...
          f"stops={p_summary['stop_steps']} jerk={p_summary['max_abs_jerk']:.2f} guard_rate={p_summary['guard_rate']*100:.1f}%]")
...
for key in ["travel_time_s", "total_fuel_mL", "stop_steps", "max_abs_jerk", "guard_rate"]:
    m, sd = summarize_metric(policy_summaries, key)
    print(f"  {key:16s}: {m:8.3f} +/- {sd:6.3f}")
...
print("\n" + "=" * 70)
print("ROUND 4 ACCEPTANCE CRITERIA")
print("=" * 70)
c1 = n_redrun == 0 and n_collision == 0
print(f"1. Zero red-runs AND zero collisions: red_run={n_redrun} collision={n_collision} -> {'PASS' if c1 else 'FAIL'}")
c2 = n_arrived >= 9
print(f"2. >=9/10 arrivals: {n_arrived}/10 -> {'PASS' if c2 else 'FAIL'}")
policy_guard_rate, _ = summarize_metric(policy_summaries, "guard_rate")
c3 = policy_guard_rate <= 0.02
print(f"3. Guard-activation rate <=2%: {policy_guard_rate*100:.2f}% -> {'PASS' if c3 else 'FAIL'}")
c4 = bool(deltas) and mean_delta <= 0.0
print(f"4. Paired fuel delta <=0%: {mean_delta:+.1f}% -> {'PASS' if c4 else 'FAIL'}")
policy_time, _ = summarize_metric(policy_summaries, "travel_time_s")
c5 = policy_time <= base_time + 10.0
print(f"5. Travel time within +10s of baseline: {policy_time:.1f}s vs {base_time:.1f}s -> {'PASS' if c5 else 'FAIL'}")
policy_stops, _ = summarize_metric(policy_summaries, "stop_steps")
c6 = policy_stops <= 20.0
print(f"6. Stop-steps <=20 (and best_model.zip confirmed loaded above): {policy_stops:.1f} -> {'PASS' if c6 else 'FAIL'}")
overall = c1 and c2 and c3 and c4 and c5 and c6
